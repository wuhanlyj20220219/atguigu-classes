 ...
 *
 */
class ConfigurationManagerScala {
  ...
  try { // 通过一个“类名.class”的方式，就可以获取到这个类在JVM中对应的Class对象
    ...
    val in: InputStream = classOf[ConfigurationManagerScala]
      .getClassLoader
  ...
  def getBoolean(key: String): Boolean = {
    ...
    try {
      ...
      val boolean: Boolean = value.toBoolean
      return boolean;
    }
    catch {
      case e: Exception =>
...
/*object ConfigurationManager {
  def main(args: Array[String]): Unit = {
    ...
    //    val string: String = manager.getBoolean("spark.local").toString
    // true
    // println(string)
